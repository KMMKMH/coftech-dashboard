...
import { useCreatePromptMutation } from "@component/store/RTK/promptsRTK";
...
const Home = ({ t }) => {
  ...
  const [triggerRestore] = useCreatePromptMutation();
  ...
  }, [data, sortConfig, search, companyFilter]);
  ...
  useEffect(() => {
    const fetchBackups = async () => {
      if (botFilter && companyFilter) {
        try {
          await trigger({ companyID: companyFilter, botID: botFilter }).unwrap()
        } catch (err) {
          handleError(err, { companyID: companyFilter, botID: botFilter })
        }
      }
    }
  ...
  const handleRestorePrompt = async (prompt, index) => {
    ...
    try {
      await triggerRestore({ companyID: prompt.company_id, botID: prompt.bot_id, data }).unwrap();
      setIsConfirmationOpen(true);
    } catch (error) {
      handleError(error, { companyID: prompt.company_id, botID: prompt.bot_id, ...data })
    } finally {
